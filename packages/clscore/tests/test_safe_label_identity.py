...
def test_two_case_variants_cannot_share_a_directory_entry():
    """The stem is a FILENAME and this is a Windows app.

    "Scratch" and "scratch" are two keys in Python and ONE entry on NTFS: the
    second .npy replaced the first, the stale sweep saw a stem that was in its
    keep-set and removed nothing, and the next load rebuilt the label dict
    from the surviving filename. A whole defect class vanished with nothing
    raised, on the ordinary tab switch that re-loads the bank.
    """
    a, b = safe_label("Scratch"), safe_label("scratch")
    assert a != b
    assert a.lower() != b.lower(), "NTFS folds case; the stems must differ anyway"


def test_every_stem_this_can_produce_is_lowercase():
    """Closure under case folding is the property that makes collisions
    impossible, not just unlikely."""
    for label in ("scratch", "Scratch", "SCRATCH", "_default", "Lot-3", "傷",
                  "傷A", "汚れA", "A B", "MiXeD_Case-1"):
        stem = safe_label(label)
        assert stem == stem.lower(), (label, stem)


def test_case_variants_stay_apart_through_an_assemble_and_a_reload(tmp_path):
    """The end the operator actually sees: two classes in, two classes back."""
    ...
    ls.assign(st.entries[1].id, tier="critical", label="Scratch")

    bank = assemble_bank(st, ls)
    assert len(bank.critical) == 2

    d = tmp_path / "bank"
    bank.save(d)
    stems = sorted(f.stem for f in (d / "critical").glob("*.npy"))
    assert len(stems) == 2, f"one .npy per class, or NTFS eats one: {stems!r}"
    assert len(set(s.lower() for s in stems)) == 2

    reloaded = Bank.load(d)
    assert len(reloaded.critical) == 2, "the class that vanished on reload"
    assert set(reloaded.critical) == set(bank.critical)
