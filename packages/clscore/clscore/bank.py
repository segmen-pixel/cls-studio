...
def safe_label(label: str | None) -> str:
    ...
    ASCII-clean label — "scratch", "dent", "lot-3" — byte-identical to what it
    produced before. That matters: those banks keep their filenames, their
    ...
    if subbed == raw:
        # Already a legal stem, so it IS the identity -- returned verbatim,
        # underscores and all. Testing the substitution rather than the
        # stripped result matters: ``.strip("_")`` would otherwise turn
        # ``_default`` into ``default-3bf30573`` and split the bucket that the
        # whole codebase special-cases by name.
        return raw
    stem = subbed.strip("_")
