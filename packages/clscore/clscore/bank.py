...
def safe_label(label: str | None) -> str:
    """Sanitise a free-text label for use as a filename stem.

    Letters/digits/underscore/hyphen are kept verbatim; anything else
    collapses to a single underscore. Empty / whitespace-only labels fall
    back to ``DEFAULT_LABEL`` so the on-disk layout is always well-defined.
    """
    if not label or not str(label).strip():
        return DEFAULT_LABEL
    s = re.sub(r"[^A-Za-z0-9_\-]+", "_", str(label).strip()).strip("_")
    return s or DEFAULT_LABEL
...
class Bank:
    ...
    def remove_images(
        ...
        for i, e in enumerate(entries):
            ...
            new_entries.append({"name": e["name"], "start": cursor, "count": count})
            cursor += count
