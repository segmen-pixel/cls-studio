...
from .fsio import atomic_handle
...
class IncidentMetaArray:
    ...
    def save(self, path: Path) -> None:
        ...
        Via ``fsio.atomic_handle``, which owns the temp naming: this file is
        written by ``Bank._save_tier`` in the same loop as the tier's feature
        ``.npy``, so two concurrent bank saves reach it together, and a temp
        name derived from the destination would be shared between them.
        """
        path = Path(path)
