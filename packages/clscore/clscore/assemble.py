...
def _carry_index(bank: Bank | None) -> dict[tuple[str, str, str, int], IncidentMetaArray]:
    """Per-image metadata slices of ``bank``, keyed by (tier, label, name, rows).
    ...
    tier.
    """
    out: dict[tuple[str, str, str, int], IncidentMetaArray] = {}
    if bank is None:
        return out
        ...
        for label, entries in index.items():
            ...
            for e in entries:
                ...
                if s < 0 or c <= 0 or s + c > len(m):
                    continue
                out[(tier, label, str(e.get("name", "")), c)] = m.take(
    ...
    *,
    ...
    for entry in store.entries:
        ...
        if a is None or a.tier not in ("normal", "critical", "negative"):
            continue
        groups.setdefault((a.tier, a.resolved_label()), []).append((entry, a))
    ...
    for (tier, label), items in groups.items():
        ...
        for (entry, assignment), budget in zip(items, budgets):
            ...
            index_entry: dict = {"name": entry.name, "start": cursor, "count": count}
            if kept is not None:
            ...
            if tier == "normal":
                ...
                donor = carry.get((tier, label, entry.name, count))
    ...
    *,
    ...
    def _carve(
        ...
        for e in entries:
            ...
            kept = e.get("kept")
            image_ref = f"_images/{tier}/{name}" if name else ""
