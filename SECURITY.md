...
>
> Cls-Studio ships with **no built-in user accounts or RBAC**. It is
...
>
> - Read, modify, and delete all project data (images, memory banks,
>   inspection results)
> - Run bank-teach and scoring jobs that consume GPU/CPU resources
...
> with TLS and authentication enabled. See
> [Reverse proxy](docs/deployment.md#reverse-proxy) in the deployment guide
> for a worked nginx / Caddy configuration.
>
> By default, all start scripts bind to `127.0.0.1` (localhost only). To
> enable LAN access, set the environment variable `CLS_HOST=0.0.0.0` --
...
```
CLS_API_TOKEN=<a long random secret>
```

in your `.env`. When present, every request to `/api/v1/*` — the entire
API surface this release ships — must carry the header
`X-API-Token: <value>` or it is rejected with `401`. Leave
...
`CLS_API_TOKEN` is primarily useful for scripted / proxy-fronted
...
Cls-Studio is designed as a **local desktop application** for Windows workstations
with NVIDIA GPUs and macOS machines with Apple Silicon (MPS). It is **not intended to be exposed to the public internet**.

- The Cls-Studio API listens on `localhost` by default.
- CORS is restricted to private network ranges (127.0.0.1, 192.168.x.x, 10.x.x.x, 172.16–31.x.x).
- No authentication is enforced unless `CLS_API_TOKEN` is configured; the
  application is otherwise assumed to run in a trusted local environment.
- The server makes **no outbound requests of its own**, with one opt-in
  exception: if `CLS_CVAT_URL` or `CLS_ANNOTATION_URL` is set, the
  matching `/cvat/*` or `/annotate/*` route is mounted and forwards requests to
  that upstream. Both are unset by default, so the routes do not exist and
  there is no server-side-request-forgery surface. Leave them unset unless an
  upstream service is genuinely running, especially when
  `CLS_HOST=0.0.0.0` exposes the API to a LAN.
...
1. **GitHub Security Advisories:** Use the [Report a vulnerability](https://github.com/segmen-pixel/cls-studio/security/advisories/new) feature on GitHub.
...
- **Upload size limits:** 200 MB per file, enforced server-side; multi-file requests (batch teach / staging drop) are additionally capped at 2 GB and 1024 files per request (`CLS_MAX_UPLOAD_TOTAL_MB` / `CLS_MAX_UPLOAD_FILES`).
