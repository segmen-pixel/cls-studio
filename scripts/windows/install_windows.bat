...
if /I "%TORCH_CUDA_INDEX%"=="cu128" (set "TORCH_PIN=torch==2.11.0") else (set "TORCH_PIN=torch==2.6.0")
"%PYTHON_EXE%" -m pip install --force-reinstall --index-url https://download.pytorch.org/whl/%TORCH_CUDA_INDEX% %TORCH_PIN% >> "%LOG_FILE%" 2>&1
...
if defined PY_BOOTSTRAP goto :resolve_python_done_py
py -3 -c "import sys; sys.exit(0 if (3, 10) <= sys.version_info[:2] <= (3, 13) else 1)" >nul 2>nul
if errorlevel 1 goto :resolve_python_no_py
...
python -c "import sys; print(sys.version_info.major)" >nul 2>nul
if errorlevel 1 goto :resolve_python_no_python
...
python3 -c "import sys; print(sys.version_info.major)" >nul 2>nul
if errorlevel 1 goto :resolve_python_auto_install
...
py -%1 -c "import sys; print(sys.version_info.major)" >nul 2>nul
if errorlevel 1 exit /b 1
...
%PY_BOOTSTRAP% -c "import sys; sys.exit(0 if (3, 10) <= sys.version_info[:2] <= (3, 13) else 1)" >nul 2>nul
if errorlevel 1 exit /b 1
exit /b 0
