...
REM ---- Select venv: CLS_VENV override; else prefer a CUDA build if present ----
REM cu128 stays in the chain so an install made before the 2026-08-17 move to
REM cu130 keeps using its GPU venv instead of silently falling back to CPU.
if not defined CLS_VENV if exist ".venv-windows-cu130\Scripts\python.exe" set "CLS_VENV=.venv-windows-cu130"
...
set "CLS_PROJECTS_DIR=%REPO_ROOT%\projects"
set "CLS_DB_PATH=%REPO_ROOT%\projects\app.db"
