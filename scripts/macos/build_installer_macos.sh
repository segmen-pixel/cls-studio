...
# cls-studio — macOS Installer Builder
#
...
#
# Output:
#   dist/cls-studio-v{VERSION}-macos-{arch}.dmg
set -euo pipefail
...
echo ""
echo "============================================================"
echo "  cls-studio macOS Installer Builder"
echo "============================================================"
...
if [ ! -d "$REPO_ROOT/apps/ui/dist" ]; then
    if command -v npm &>/dev/null; then
        ...
        (cd "$REPO_ROOT/apps/ui" && npm install && npm run build)
        ok "UI built"
    else
        fail "UI not pre-built and npm not available. Run 'cd apps/ui && npm run build' first."
    fi
else
...
ok "Build complete!"
echo ""
echo "  Output: dist/cls-studio-v${VERSION}-macos-*.dmg"
echo ""
echo "  To test the .dmg:"
echo "    1. Double-click the .dmg file"
echo "    2. Drag cls-studio to Applications"
