...
# cls-studio -- Stop Local Services (macOS)
set -euo pipefail
...
info "Stopping cls-studio local services..."

# ── Port-based kill ──────────────────────────────────────────────
STOPPED=0
for entry in "8791:cls-studio API"; do
    PORT="${entry%%:*}"
    NAME="${entry#*:}"
...
if [ "$STOPPED" -eq 0 ]; then
    info "No services found listening on port 8791."
else
