...
def _setup_environment(root: Path) -> Path:
    ...
    default_projects = _home / "Documents" / "ClsStudio" / "projects"
    projects_dir = Path(os.environ.get("CLS_PROJECTS_DIR", str(default_projects)))
    os.environ.setdefault("CLS_PROJECTS_DIR", str(projects_dir))

    # ---- Database path (matches existing convention: projects/app.db) ----
    db_path = projects_dir / "app.db"
    os.environ.setdefault("CLS_DB_PATH", str(db_path))
...
def _resolve_bind_host(projects_dir: Path) -> str:
    ...
    Resolution order:
      1. CLS_HOST environment variable (if set, wins — preserves legacy behaviour).
      2. runtime_settings.json `lan_access: true` → "0.0.0.0".
      3. Default → "127.0.0.1" (loopback-only).
    """
    env_host = (os.environ.get("CLS_HOST") or "").strip()
    if env_host:
        return env_host
...
def _find_port(start: int = 8791, end: int = 8810) -> int:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="cls-studio Launcher")
    parser.add_argument(
        "--no-browser",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Preferred port (default: auto-select from 8791-8810)",
    )
    ...
    print(f"[launcher] cls-studio v{version}")
    ...
    print(f"[launcher] Database        : {os.environ['CLS_DB_PATH']}")
    ...
    os.environ.setdefault("CLS_HOST", host)
    ...
    print(f"[launcher] Starting cls-studio v{version} on port {port}")
