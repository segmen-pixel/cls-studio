...
Exit codes: 0 = clean, 1 = denied or unresolvable (allowlist to accept)
...
def main() -> int:
    ...
    if denied:
        ...
        return 1
    if unknown:
        # Fatal, not a warning. A dependency whose licence we cannot resolve is
        # exactly the case this gate exists for — warning on it means the one
        # package nobody classified is the one that ships. Put a reviewed
        # package in the allowlist to make it pass deliberately.
        print(f"::error::{len(unknown)} package(s) with unresolvable license metadata:")
        for u in unknown:
            print(f"  UNKNOWN {u}")
        print("  add reviewed packages to the --allowlist file to accept them")
        return 1
    print("OK - no blocked or unresolvable licenses")
    return 0
