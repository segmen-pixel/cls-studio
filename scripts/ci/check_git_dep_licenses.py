...
def extract_git_deps(files: list[str]) -> dict[str, str]:
    ...
    for f in files:
        p = Path(f)
        if not p.exists():
            # Hard error, not a skip: with the zero-deps outcome now a PASS,
            # a typo'd path here would otherwise sail through silently.
...
def main() -> int:
    ...
    if not deps:
        # Divergence from the seg-studio original, which fails here: seg has
        # git-pinned deps (mobile-sam), so zero extractions there means the
        # file list is wrong. cls-studio has no git pins today, so zero is
        # the expected, verified-clean outcome. Missing input files still
        # fail above, which keeps the typo case fail-closed.
        print("OK - scanned inputs contain no git+URL dependencies; "
              "nothing to vet (gate arms itself the day one is added)")
        return 0
