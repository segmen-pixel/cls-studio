...
# ---------------------------------------------------------------
# cls-studio — Dependency Security Audit (cross-platform)
...
if [ -f "$UI_DIR/package-lock.json" ]; then
    ...
    echo "SKIP: package-lock.json not found in apps/ui"
fi
echo ""
...
if [ -x "$REPO_ROOT/.venv/bin/python" ]; then
    ...
    PYTHON="$REPO_ROOT/.venv-windows-cu130/Scripts/python.exe"
