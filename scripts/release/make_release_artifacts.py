...
A release is more than the installer. Someone who will not run a large download
unverified needs a digest to check it against; someone on a platform we ship no
binary for needs to be told that in the same place they looked for one; and the
copyleft libraries inside the binary carry a source-offer obligation that is
only discharged if the sources are attached to the same Release as the binary.

    python scripts/release/make_release_artifacts.py --version 0.2.0 --ref v0.2.0

Produces, in dist/v<version>/:

    Cls-Studio-v<version>-win64.zip    built earlier by build_installer.py
    cls-studio-v<version>-source.zip   git archive of --ref
    lgpl-sources-v<version>.zip        sources for the shipped LGPL/MPL DLLs
    SHA256SUMS.txt                     one line per file, sha256sum -c format

There is deliberately no macOS binary. The macOS wheel of
opencv-python-headless bundles a GPL FFmpeg build, so shipping a macOS bundle
would mean an Apache-2.0 artifact redistributing GPL binaries; macOS installs
from source instead and pip fetches that wheel on the user's own machine.
build_installer.py refuses to package it -- see _assert_no_gpl_binaries().
"""
...
DEV_REMOTE_MARKERS = ("cls-studio-dev",)
...
def _source_archive(version: str, ref: str, out_dir: Path) -> Path:
    ...
    out = out_dir / f"cls-studio-v{version}-source.zip"
    _run([
        "git", "-C", str(ROOT), "archive", "--format=zip",
        f"--prefix=cls-studio-v{version}/", "-o", str(out), ref,
    ])
    return out
...
def main() -> int:
    ...
    ap.add_argument("--version", required=True, help="e.g. 0.2.0")
