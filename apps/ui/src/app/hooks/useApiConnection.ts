...
export function useApiConnection(showToast: (msg: string) => void) {
  ...
  const refreshProjects = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // Prune a stale selection (deleted project surviving in
      // sessionStorage): keeping it strands the Develop/Operator tabs on a
      // 404 "bank select failed" with no recovery path.
      setSelectedProjectId((prev) => {
        if (prev && summaries.some((s) => s.id === prev)) return prev;
        if (prev) sessionStorage.removeItem("seg-project");
