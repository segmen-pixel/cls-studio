...
/** The (project, bank) this client is bound to, or null before the first
 *  select resolves. Exposed so per-bank UI state can be filed under the same
 *  identity the server guards writes with, rather than a second one that can
 *  disagree with it. */
export function activeBindingKey(): string | null {
...
/** Download URL for the active bank as an on-device inspection package.
 *
 * Not the same thing as bankExportUrl: that one round-trips a bank between
 * installs of this program, this one is what a device scores with. Features
 * quantised to int8, the IVF centroids, the defect exemplars and the manifest
 * that says which encoder produced them -- no source images, no fp16. */
export function bankEdgeExportUrl(): string {
  return `${API_BASE}/bank/export/edge`;
}

/** Whether this server can produce a Core ML encoder, and why not if it cannot.
 *
 * Two flags, not one: converting and being able to run the result are
 * different capabilities, and a conversion that cannot be run cannot be
 * checked against the bank it has to match. Off macOS the honest answer is
 * usually "no", which is why the UI asks before offering the control. */
export type CoremlCapability = {
  available: boolean;
  convert: boolean;
  predict: boolean;
  reason: string | null;
...
export function coremlCapability(signal?: AbortSignal): Promise<CoremlCapability> {
  return jget<CoremlCapability>("/hardware/coreml", signal);
}

/** Whether this server can produce an OpenVINO IR. Same shape as the Core ML
 * one; unlike it, the two flags move together, because OpenVINO converts and
 * runs on the same machine. */
export function openvinoCapability(signal?: AbortSignal): Promise<CoremlCapability> {
  return jget<CoremlCapability>("/hardware/openvino", signal);
}

/** Download URL for an OpenVINO IR encoder matching the active bank. */
export function openvinoIrUrl(precision: "fp32" | "fp16" = "fp32"): string {
  return `${API_BASE}/bank/export/openvino?precision=${precision}`;
}

/** Download URL for an encoder matching the active bank. */
export function coremlEncoderUrl(precision: "float32" | "float16" = "float32"): string {
  return `${API_BASE}/bank/export/coreml?precision=${precision}`;
}
...
// ---------------------------------------------------------------------------
// Import format (how a source image is stored on disk)
// ---------------------------------------------------------------------------

export type ImportFormat = "raw" | "png" | "jpg";
...
/** How imported source images are stored. Applies to the next import only. */
export function fetchImportSettings(): Promise<ImportSettings> {
  return jget<ImportSettings>("/system/import");
}

export function updateImportSettings(
  settings: { format: ImportFormat; jpg_quality: number },
): Promise<ImportSettings> {
  return apiPost<ImportSettings>("/system/import", settings, { method: "PUT" });
}
