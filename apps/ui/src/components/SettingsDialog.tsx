...
/** How an imported source image is stored. The stored format used to follow
 *  whatever the camera emitted; a line that wants the pixels untouched and a
 *  line that wants the disk to last the year now each get to say so. */
function ImportFormatSection() {
  const { t } = useI18n();
  const [state, setState] = useState<ImportSettings | null>(null);
  const [busy, setBusy] = useState(false);
  const [savedMsg, setSavedMsg] = useState<string | null>(null);

  useEffect(() => {
    fetchImportSettings().then(setState).catch(() => {});
  }, []);

  const apply = async (next: { format: ImportFormat; jpg_quality: number }) => {
    if (busy) return;
    setBusy(true);
    setSavedMsg(null);
    try {
      setState(await updateImportSettings(next));
      setSavedMsg(t("settings.import.saved"));
      setTimeout(() => setSavedMsg(null), 6000);
    } catch (err) {
      setSavedMsg(`Failed: ${(err as Error).message}`);
    } finally {
  ...
  return (
    <section className="settings-section-divider">
      <h3 style={{ color: "#4fc3f7" }}>🖼 {t("settings.import")}</h3>
      <label style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2, fontSize: TYPE.xs }}>
        {t("settings.import.format")}
        <select
          value={state.format}
          disabled={busy}
          onChange={(e) => apply({ ...state, format: e.target.value as ImportFormat })}
          style={{ fontSize: TYPE.xs, padding: "2px 6px" }}
        >
          {state.formats.map((f) => (
            <option key={f} value={f}>{t(`settings.import.format.${f}`)}</option>
          ))}
        </select>
      </label>
      <p className="muted" style={{ fontSize: TYPE.xs, marginBottom: 8 }}>
        {t(`settings.import.format.${state.format}.desc`)}
      </p>
      {state.format === "jpg" && (
        <label style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, fontSize: TYPE.xs }}>
          {t("settings.import.quality")}
          <select
            value={state.jpg_quality}
            disabled={busy}
            onChange={(e) => apply({ ...state, jpg_quality: parseInt(e.target.value, 10) })}
            style={{ fontSize: TYPE.xs, padding: "2px 6px" }}
          >
            {[60, 75, 85, 92, 97, 100].map((q) => (
              <option key={q} value={q}>{q}</option>
            ))}
          </select>
        </label>
      )}
      <p className="muted" style={{ fontSize: TYPE.xs, marginBottom: 4 }}>
        {t("settings.import.appliesNext")}
      </p>
      {savedMsg && (
        <div style={{ fontSize: TYPE.xs, color: savedMsg.startsWith("Failed") ? "#ef9a9a" : "#a5d6a7" }}>{savedMsg}</div>
      )}
...
/** VRAM budget + idle release. Both knobs answer the same complaint: the
 *  server sizes its scoring buffers against whatever is free on the card and
 *  the allocator never gives that back, so a neighbouring job (a training run
 *  on the same GPU) finds nothing left. */
function VramSection() {
  const { t } = useI18n();
  const [state, setState] = useState<(VramSettings & { usage: VramUsage }) | null>(null);
  const [busy, setBusy] = useState(false);
  const [savedMsg, setSavedMsg] = useState<string | null>(null);

  useEffect(() => {
    fetchVramSettings().then(setState).catch(() => {});
  }, []);

  const apply = async (next: VramSettings) => {
    if (busy) return;
    setBusy(true);
    setSavedMsg(null);
    try {
      setState(await updateVramSettings(next));
      setSavedMsg(t("settings.vram.saved"));
      setTimeout(() => setSavedMsg(null), 6000);
    } catch (err) {
      setSavedMsg(`Failed: ${(err as Error).message}`);
    } finally {
  ...
  const releaseNow = async () => {
    ...
    try {
      ...
      // "busy" is a normal answer, not a failure: the server is working and
      // freeing a referenced block would not have worked anyway.
      setSavedMsg(
        r.released
          ? t("settings.vram.released").replace("{mb}", String(r.freed_mb ?? 0))
          : t("settings.vram.releaseBusy"),
      );
      setTimeout(() => setSavedMsg(null), 6000);
      setState(await fetchVramSettings());
    } catch (err) {
      setSavedMsg(`Failed: ${(err as Error).message}`);
    } finally {
      setBusy(false);
  ...
  return (
    <section className="settings-section-divider">
      <h3 style={{ color: "#4fc3f7" }}>🎛 {t("settings.vram")}</h3>
      <label style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2, fontSize: TYPE.xs }}>
        {t("settings.vram.budget")}
        <select
          value={state.budget_mb}
          disabled={busy}
          onChange={(e) => apply({ ...state, budget_mb: parseInt(e.target.value, 10) })}
          style={{ fontSize: TYPE.xs, padding: "2px 6px" }}
        >
          {[0, 2048, 4096, 6144, 8192, 12288, 16384].map((n) => (
            <option key={n} value={n}>
              {n === 0 ? t("settings.vram.budget.unlimited") : `${n / 1024} GB`}
            </option>
          ))}
        </select>
      </label>
      <p className="muted" style={{ fontSize: TYPE.xs, marginBottom: 8 }}>
        {t("settings.vram.budget.desc")}
      </p>
      <label style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
        <input
          type="checkbox"
          checked={state.idle_release}
          disabled={busy}
          onChange={(e) => apply({ ...state, idle_release: e.target.checked })}
        />
        <span>{t("settings.vram.idle")}</span>
      </label>
      <p className="muted" style={{ fontSize: TYPE.xs, marginBottom: 8 }}>
        {t("settings.vram.idle.desc")}
      </p>
      {state.idle_release && (
        <label style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, fontSize: TYPE.xs }}>
          {t("settings.vram.idleSeconds")}
          <select
            value={state.idle_seconds}
            disabled={busy}
            onChange={(e) => apply({ ...state, idle_seconds: parseInt(e.target.value, 10) })}
            style={{ fontSize: TYPE.xs, padding: "2px 6px" }}
          >
            {[30, 60, 300, 900, 1800].map((n) => (
              <option key={n} value={n}>{n < 60 ? `${n} s` : `${n / 60} min`}</option>
            ))}
          </select>
        </label>
      )}
      <label style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
        <input
          type="checkbox"
          checked={state.drop_bank}
          disabled={busy}
          onChange={(e) => apply({ ...state, drop_bank: e.target.checked })}
        />
        <span>{t("settings.vram.dropBank")}</span>
      </label>
      <p className="muted" style={{ fontSize: TYPE.xs, marginBottom: 8 }}>
        {t("settings.vram.dropBank.desc")}
      </p>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        <button className="ghost" disabled={busy} onClick={releaseNow} style={{ fontSize: TYPE.xs }}>
          {t("settings.vram.releaseNow")}
        </button>
        <span className="muted" style={{ fontSize: TYPE.xs }}>
          {state.usage?.available
            ? `${state.usage.reserved_mb} MB ${t("settings.vram.reserved")}`
            : t("settings.vram.noCuda")}
        </span>
      </div>
      {savedMsg && (
        <div style={{ fontSize: TYPE.xs, color: savedMsg.startsWith("Failed") ? "#ef9a9a" : "#a5d6a7" }}>{savedMsg}</div>
  ...
  return (
    <div className="settings-overlay" onClick={onClose} onTouchEnd={(e) => { if (e.target === e.currentTarget) onClose(); }} onTouchMove={(e) => e.stopPropagation()}>
      <div className="settings-panel" onClick={(e) => e.stopPropagation()} onTouchEnd={(e) => e.stopPropagation()}>
        ...
        <CompressionSection />
        {/* Stored format for imported images */}
        <ImportFormatSection />
