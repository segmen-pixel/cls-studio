...
import { ACCENT, BORDER, DANGER, INK, MUTED, OK, PANEL_2, RULE, TYPE } from "../ui/tokens";
...
export default function Operator({ projectId, active, showToast }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // thr must reset with the rest: without this, a project with no saved
    // config and no cached evals silently judges restored inspections
    // against the PREVIOUS project's threshold.
    setEvals([]); setFromConfig(false); setCfgStale(false); setThr(null); thrEdited.current = false;
  ...
  const pump = useCallback(async () => {
    ...
    try {
      while (queueRef.current.length) {
        ...
        abortRef.current = ac;
        try {
          const r = await scoreImage(job.file, { k: 5, topk: TOPK, alpha: alphaRef.current, beta: betaRef.current, signal: ac.signal });
