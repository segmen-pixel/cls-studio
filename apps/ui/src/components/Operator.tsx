...
import { readScopedNumber, writeScopedNumber } from "../app/scopedStorage";
...
export default function Operator({ projectId, active, showToast }: Props) {
  ...
  // α is shared with the Teach tab's slider so the value proven on the
  // separation check is what inspection runs with -- but filed per bank, since
  // it weights that bank's exemplars and means nothing against another's.
  ...
  // Re-read when the bank changes rather than once at mount: at mount the
  // binding is usually still null, and a value read under the old bank is the
  // bug this scoping exists to remove.
  const binding = activeBindingKey();
  useEffect(() => {
    const a = readScopedNumber("anom.alpha", binding);
    const b = readScopedNumber("anom.beta", binding);
    setAlpha(a !== null && a > 0 ? Math.min(a, ALPHA_MAX) : 0);
    setBeta(b !== null && b > 0 ? Math.min(b, ALPHA_MAX) : 0);
  }, [binding]);
  useEffect(() => { writeScopedNumber("anom.alpha", binding, alpha); }, [binding, alpha]);
  useEffect(() => { writeScopedNumber("anom.beta", binding, beta); }, [binding, beta]);
  ...
  useEffect(() => {
    ...
    const savedAlpha = readScopedNumber("anom.alpha", activeBindingKey()) ?? NaN;
    if (Number.isFinite(savedAlpha)) setAlpha(Math.min(Math.max(savedAlpha, 0), ALPHA_MAX));
    const savedBeta = readScopedNumber("anom.beta", activeBindingKey()) ?? NaN;
