...
export default function BankProjection({
  ...
  // Filed per bank. Re-read when the bank changes rather than once at mount:
  // at mount the binding is usually still null, and the value that used to
  // load there was whatever the previous project had been tuned to.
  const binding = activeBindingKey();
  useEffect(() => {
    const v = readScopedNumber("anom.alpha", binding);
    ...
    setAlpha(v !== null && v > 0 ? Math.min(v, ALPHA_MAX) : 0);
  }, [binding]);
