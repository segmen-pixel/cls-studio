...
/**
 * How one image fell against the check's threshold.
 *
 * - `ok`   normal tier, at or under the threshold
 * - `fp`   normal tier, over it — a false alarm
 * - `ng`   critical tier, over it
 * - `miss` critical tier, at or under it
 * - `neg`  negative tier: suppression memory, not a side of this separation,
 *          so it carries a score but no verdict mark
 */
export type EvalVerdictKind = "ok" | "ng" | "fp" | "miss" | "neg";
export type EvalVerdict = { v: number; kind: EvalVerdictKind };

/**
 * Keyed by `tier/label/name`, the same key the sweep results use.
 *
 * Built where the threshold is decided and handed to the image list, rather
 * than recomputed there: a second derivation is a second threshold, and it
 * would disagree with the histogram the moment the slider moved.
 */
export type EvalVerdicts = Map<string, EvalVerdict> | null;
