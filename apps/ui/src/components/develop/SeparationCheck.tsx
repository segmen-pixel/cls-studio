...
export default function SeparationEval({
  ...
  // The store entry, when the bank has one. Keyed on the name, two images
  // sharing a filename wrote into ONE result slot: the second overwrote the
  // first and the histogram, the AUROC and the result column all counted a
  // pair as a single image. Banks assembled before the id fall back to the
  // composite, which is what they have.
  const keyOf = (im: { tier: Tier; label: string; name: string; entry_id?: string }) =>
  ...
  useEffect(() => {
    ...
    fetchCachedEvaluations()
      .then((rows) => {
        ...
        // Same key the sweep writes. Keyed by name here while the sweep keyed
        // by store entry, this effect re-ran the moment the sweep finished
        // (it watches `running`) and overwrote every fresh result with a
        // differently-keyed copy -- the result column went back to a dash the
        // instant it filled in. Caught by teach-list.spec.
  ...
  const run = useCallback(async (): Promise<boolean> => {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        const r = await evaluateBankImage(
          im.tier, im.name, im.label, grouping, ac.signal, im.entry_id ?? "");
