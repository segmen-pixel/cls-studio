...
const ProjectTile: React.FC<ProjectTileProps> = ({
  ...
  return (
    <div
      ...
      onDoubleClick={() => openProjectWorkspace(project.id, "bank")}
      role="button"
      tabIndex={0}
      ...
      onKeyDown={(event) => {
        if (event.key === " " || event.key === "Spacebar") {
          ...
          event.preventDefault();
          if (selectedProjectId === project.id) openProjectWorkspace(project.id, "bank");
    ...
    >
      ...
      <div className="project-tile-actions">
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            // A plain navigation, not fetch(): the package runs to GB and the
            // browser should stream it to disk rather than buffer it in a blob.
            window.location.href = `${API_BASE}/projects/${project.id}/export`;
          }}
          title={t("projects.exportDesc")}
