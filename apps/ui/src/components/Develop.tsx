...
export default function Develop({ projectId, active, showToast }: Props) {
  ...
  // Cache-bust key for every image URL this tab builds. The BANK id alone is
  // not unique -- DEFAULT_BANK_ID is "default" in every project (cls_state)
  // and store entry ids are a per-store counter -- so filename + id + bank
  // collided across projects. These URLs are not decorative: the two fetches
  // below feed the bytes into scoreImage, so a collision meant the heatmap
  // and max/p99/mean could be computed from another project's pixels.
  const imgOwner = `${projectId ?? ""}/${activeBankId}`;
  ...
  // Asked once rather than on click: a control that is going to refuse should
  // look refused, not fail after the operator commits to a large download.
  const [coreml, setCoreml] = useState<{ available: boolean; reason: string | null } | null>(null);
  const [openvino, setOpenvino] = useState<{ available: boolean; reason: string | null } | null>(null);
  useEffect(() => {
    const ac = new AbortController();
    coremlCapability(ac.signal).then(setCoreml).catch(() => setCoreml(null));
    openvinoCapability(ac.signal).then(setOpenvino).catch(() => setOpenvino(null));
    return () => ac.abort();
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    // Both early returns have to clear the spinner, not just the async path.
    // A run that is superseded never reaches its own finally -- the cleanup
    // sets cancelled first -- so whatever it left on stays on. Switching away
    // from an image mid-score and back to one already cached took the cache
    // hit below, returned, and left the overlay up for good: inset 0, so the
    // heatmap it had just set was underneath it the whole time.
    if (!preview || !heatOn) { setHeatResult(null); setHeatLoading(false); return; }
    const key = imageKey(preview);
    const hit = heatCache.current.get(key);
    if (hit) { setHeatResult(hit); setHeatLoading(false); return; }
    let cancelled = false;
    setHeatLoading(true); setHeatResult(null);
    (async () => {
      try {
        const res = await fetch(imageUrl(preview.url, imgOwner));
    ...
    return () => { cancelled = true; };
  }, [preview, heatOn, imgOwner, heatAnchors, showToast]);
    ...
    <div ref={tabRef} className="develop-tab" style={{ display: "flex", height: "100%", minHeight: 0 }}>
      ...
      {/* Centre: the image, floor to ceiling. The check used to sit above it
        ...
        <div data-tutorial-step="develop-viewer" style={{ border, borderRadius: 10, padding: 12, display: "flex", flexDirection: "column", gap: 12, minWidth: 0, minHeight: 0, flex: 1 }}>
          ...
          {/* Model artifacts, and only those: what a device or another runtime
              needs to score with this bank. Moving a project between machines
              is a different job with a different scope -- every bank, the
              images, the masks, the log -- and it has its own control on the
              project tile. A zip labelled "the bank" sat between the two
              meanings and read as neither. */}
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", flex: "none" }}>
            ...
            <span style={{ marginLeft: "auto", display: "flex", gap: 6, flex: "none" }}>
              <button
                onClick={() => { window.location.href = bankEdgeExportUrl(); }}
                disabled={!bank || bank.normal === 0}
                title={t("bank.edgeExportHint")}
                style={{ padding: "5px 10px", fontSize: TYPE.sm, borderRadius: 6, border: RULE, background: "transparent", cursor: !bank || bank.normal === 0 ? "not-allowed" : "pointer", opacity: !bank || bank.normal === 0 ? .4 : 1 }}
              >⬇ {t("bank.edgeExport")}</button>
              <button
                onClick={() => { window.location.href = openvinoIrUrl("fp16"); }}
                disabled={!bank || bank.normal === 0 || openvino?.available !== true}
                title={openvino && !openvino.available
                  ? (openvino.reason ?? t("bank.ovUnavailable"))
                  : t("bank.ovExportHint")}
                style={{ padding: "5px 10px", fontSize: TYPE.sm, borderRadius: 6, border: RULE, background: "transparent", cursor: openvino?.available === true ? "pointer" : "not-allowed", opacity: !bank || bank.normal === 0 || openvino?.available !== true ? .4 : 1 }}
              >⬇ {t("bank.ovExport")}</button>
              <button
                onClick={() => { window.location.href = coremlEncoderUrl(); }}
                disabled={!bank || bank.normal === 0 || coreml?.available !== true}
                title={coreml && !coreml.available
                  ? (coreml.reason ?? t("bank.coremlUnavailable"))
                  : t("bank.coremlExportHint")}
                style={{ padding: "5px 10px", fontSize: TYPE.sm, borderRadius: 6, border: RULE, background: "transparent", cursor: coreml?.available === true ? "pointer" : "not-allowed", opacity: !bank || bank.normal === 0 || coreml?.available !== true ? .4 : 1 }}
              >⬇ {t("bank.coremlExport")}</button>
            </span>
          </div>
          {preview ? (
            <>
              ...
              >
                ...
                <img
                  draggable={false}
                  src={heatOn && heatResult ? `data:image/png;base64,${heatResult.heatmap_png_base64}` : imageUrl(preview.url, imgOwner)}
                  alt={preview.name}
