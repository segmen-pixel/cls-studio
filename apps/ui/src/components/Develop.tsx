...
export default function Develop({ projectId, active, showToast }: Props) {
  ...
  const verdictOf = useCallback(
    (im: BankImage) => evalVerdicts?.get(`${im.tier}/${im.label}/${im.name}`),
    [evalVerdicts]);
   ...
   *
   * The list holds bank images (tier/label/name); assignment is by store id,
   * so the two are matched on name AND tier through the store listing. A name
   * that resolves to more than one store entry is left alone rather than
   * guessed at: the store allows duplicate names on purpose, and picking one
   * would silently label the wrong image.
   */
