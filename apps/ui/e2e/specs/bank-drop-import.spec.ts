...
test("a multi-chunk import repaints the grid as it goes", async ({ page, request }) => {
  const proj = await createProject(request, uniqueName("chunks"));
  try {
    await selectBank(request, proj.id);
    await page.goto("/");
    await page.getByText(proj.name).first().click();
    ...
    const bank = page.locator(".bank-tab");
    await expect(bank).toBeVisible({ timeout: 30_000 });
    await expect(page.getByRole("button", { name: ALL_CHIP })).toHaveText(/\b0$/, {
      timeout: 15_000,
    });

    // Counting the listing fetches is the deterministic form of "the grid
    // updates while the import runs": watching the number tick is a race, but
    // the requests behind each repaint are not. INGEST_CHUNK is 24, so 30
    // files are two chunks.
    let listings = 0;
    page.on("request", (r) => {
      if (r.method() === "GET" && /\/api\/v1\/store(\?|$)/.test(r.url())) listings += 1;
    });

    const names = Array.from({ length: 30 }, (_, i) => `e2e-chunk-${i}.png`);
    const dt = await fileDataTransfer(page, names);
    await bank.dispatchEvent("drop", { dataTransfer: dt });

    await expect(page.getByRole("button", { name: ALL_CHIP }))
      .toHaveText(/\b30$/, { timeout: 180_000 });

    // Exactly one repaint per chunk, and no more: the old code fetched the
    // listing once, at the end, against a frozen grid. The tail used to add a
    // third fetch until onImport stopped calling the full refresh() — the
    // last chunk has already brought the listing, so only the label sets and
    // the assembly status still need a look.
    expect(listings, `listing fetches during the import: ${listings}`).toBe(2);
  } finally {
    await deleteProject(request, proj.id);
  }
});
