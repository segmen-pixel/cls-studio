...
// BSD/MIT terms of bundled npm packages (react, react-dom, ...)
...
export default defineConfig(({ mode }) => ({
  ...
  server: {
    ...
    strictPort: true,
    proxy: {
      "/api": { target: "http://localhost:8791", ws: true },
      "/v2": "http://localhost:8791",
      "/ws": { target: "ws://localhost:8791", ws: true },
      "/health": "http://localhost:8791",
      "/version": "http://localhost:8791",
      "/startup-status": "http://localhost:8791",
    },
  },
}));
