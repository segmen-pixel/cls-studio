...
"""Structured logging configuration for the cls-studio API.
...
def configure_logging(log_dir: Path | None = None) -> None:
    ...
    # --- stdout handler ---
    # The file handlers below already say errors="backslashreplace". The
    # stream did not, and it is the one that runs everywhere -- including on a
    # Japanese Windows console, where the codec is cp932 and U+2014 EM DASH
    # has no mapping. This project's own prose uses em dashes, so they reach
    # log messages: "no active project - POST /api/v1/bank/select first" is
    # one, and emitting it raised UnicodeEncodeError inside the handler.
    # logging swallows that and keeps serving, so nothing looks broken from
    # outside; the log simply stops, which is worse than a loud failure
    # because the next person debugging has no record at all.
    #
    # Only the error policy changes. Forcing UTF-8 as well would garble the
    # Japanese that a cp932 console renders correctly today; backslashreplace
    # keeps every character that fits and escapes the handful that do not.
