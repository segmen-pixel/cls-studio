...
def _cleanup_orphan_project_dirs() -> None:
    ...
    from .paths import looks_like_project_id as _looks_like_project_id
    engine = get_engine()
    ...
    for entry in PROJECTS_DIR.iterdir():
        if not entry.is_dir():
            continue
        if not _looks_like_project_id(entry.name):
            continue
        if entry.name in db_ids:
            continue
        pj_path = entry / "project.json"
        tombstone = entry / ".deleted"
        # "banks" / "captures" / "inspections" are content too: an cls-studio
        # project that lost its project.json (crash between create steps,
        # partial delete without tombstone) but still holds taught banks or
        # operator records must be adopted, never purged — irreplaceable
        # user data.
        has_content = any(
            (entry / sub).exists()
            for sub in ("images", "masks", "index.json", "training",
                        "banks", "captures", "inspections")
        )
        # The sweep runs while the API is already serving: a project being
        # created right now has its dirs on disk before its DB row commits.
        # Anything modified in the last few minutes is skipped — the next
        # startup reconciles it with a settled DB.
        try:
            import time as _time
            if _time.time() - entry.stat().st_mtime < 300:
                continue
        except OSError:
            continue
...
def _cleanup_stale_runs_on_startup() -> None:
    ...
    with Session(engine) as session:
        ...
        #    (directories with model.pt/anomaly_model.pkl are kept — they are
        #    discoverable via _discover_fs_runs and usable from the UI)
...
def _run_health_check() -> None:
    ...
    libs = [
        ...
        ("sklearn",      "sklearn",      "Coreset / projection"),
    ]
...
def _auto_build_ui() -> None:
    """Rebuild ui if dist/ is missing or stale (source newer than build)."""
    import subprocess
    ui_dir = ROOT_DIR / "apps" / "ui"
...
def _check_inference_deps(resolved_device: str, startup_state: dict) -> None:
    """Check cls-studio inference dependencies and warn if anything is missing.

    cls-studio needs torch (+ the DINOv2 hub backbone, fetched on first use).
    Predecessor checks for annotation-model checkpoints and cloud-training
    libraries are gone — neither feature exists here. We surface a single
    warning only if torch itself is unavailable, which would break all
    scoring.
    """
    try:
        import torch  # noqa: F401
        logger.info("torch OK")
    except ImportError:
        startup_state["warnings"].append({
            "level": "warning",
            "title": "PyTorch not installed",
            "message": "cls-studio needs PyTorch to extract features. Install the "
                       "project requirements before teaching or scoring.",
        })
        logger.warning("torch not installed --scoring will not work")

    is_cuda = resolved_device.startswith("cuda")
    if not is_cuda:
        return  # CPU-only setup --nothing more to warn about

    # cls-studio scores through torch (optionally OpenVINO on CPU); there is no
    # ONNX Runtime inference path, so torch is the only CUDA consumer to check.
    torch_cuda = False
    try:
        import torch
    ...
    if not torch_cuda:
        startup_state["warnings"].append({
            ...
            "message": (
                f"Device {resolved_device} is selected, but PyTorch reports no "
                "usable CUDA device.\n"
                "Scoring will fall back to CPU and will be far slower.\n\n"
                "Fix: verify the PyTorch CUDA build "
                "(torch.cuda.is_available() should return True)."
            ),
        })
        logger.warning("torch CUDA unavailable --scoring will be CPU-only")
    else:
        logger.info("Inference deps OK: torch CUDA available")
...
def _deferred_post_startup() -> None:
    ...
    try:
        _run_health_check()
        _auto_check_deps()
        # After ready, deliberately: the reaper must never sit in front of the
        # UI build, and it has nothing to do until something loads torch.
        from .vram_reaper import start_vram_reaper

        start_vram_reaper()
        # Dropped for cls-studio: _scan_all_projects_integrity (segmentation
        # mask/class integrity) and cloud-training job recovery.
    except Exception:
