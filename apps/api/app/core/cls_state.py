...
class ClsStudioState:
    ...
    def save_source_image(self, tier: str, filename: str, data: bytes) -> str:
        ...
        # The stored format is a setting, and core/import_format.py owns both
        # the setting and the decision -- the store ingest route calls the same
        # function, so the two cannot drift the way they had.
        from .import_format import encode_source_image

        safe, data = encode_source_image(filename, data)
        target = d / safe
