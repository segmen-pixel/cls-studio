...
from clscore.fsio import atomic_handle
...
from .cls_state import ClsStudioState, gpu_leased
from .exceptions import ValidationError
from .paths import get_path_lock
...
def active_labelset(state: ClsStudioState, create: bool = True) -> LabelSet:
    ...
    Serialised per label-sets directory. Opening a project fires
    ``/bank/select``, ``/labelsets`` and ``/store`` on mount, FastAPI runs those
    sync endpoints in a threadpool, and on a *fresh* project all three reach the
    creation branch below -- three threads each writing ``standard.json`` in the
    belief that they were the one creating it. Unique temp names
    (``clscore.fsio``) stop that from failing outright, but the read that
    decides to create and the writes that create belong in one critical
    section, not in a race to be last.

    The lock is per directory, so two projects never wait on each other.
    """
    d = labelsets_dir(state)
    with get_path_lock(d):
...
def _write_store_image(store: FeatureStore, entry: StoreEntry, filename: str, data: bytes) -> str:
    ...
    The format is the operator's choice, and core/import_format.py owns both
    the setting and the transcode -- the teach route calls the same function.
    The entry id, not the operator's filename, still names the file; only the
    extension follows the encoder, so the two stay in step by construction.
    """
    from .import_format import encode_source_image

    encoded_name, data = encode_source_image(filename, data)
    name = f"{entry.id}{Path(encoded_name).suffix or '.png'}"
    d = store.images_dir()
...
def rendered_image(src: Path, cache_dir: Path, entry_id: str, size: str) -> Path:
    ...
    try:
        ...
        # Unlocked, and two requests for one rendition arrive together often
        # enough -- the grid asks for every tile at once, and a reload re-asks
        # before the first pass has finished. A temp named after the
        # destination is the collision fsio was written to end: both renders
        # create it, the first replace moves it away, the second finds its own
        # source gone. The loser is swallowed below and silently serves the
        # full-size original, so it showed up as a slow grid, never an error.
        with atomic_handle(out) as fh:
    ...
    *,
    ...
    for i in range(0, len(decoded), INGEST_GROUP):
        ...
        for (fname, data, arr), (feats, kept) in zip(grp, produced):
            ...
            entry.image_ref = _write_store_image(store, entry, fname, data)
            out.append(entry)
