...
class StoredImageEval(BaseModel):
    ...
    entry_id: str = Field(
        "",
        description=(
            "Store entry these numbers describe. The filename is not an "
            "identity -- two entries may share one -- so a client keying its "
            "result map by name collapses a duplicate pair into one row. "
            "Empty for banks assembled before the id was recorded."
        ),
    )
    patches: int
    score_max: float
...
class AssemblyStatus(BaseModel):
    ...
    assigned: int = 0
    unassigned: int = 0
    stale_assignments: int = Field(
        0,
        description=(
            "Assignments naming store entries that no longer exist. Counted "
            "rather than absorbed: the old arithmetic subtracted two totals "
            "and clamped at zero, so a divergence read as 'nothing left to "
            "label' instead of surfacing itself."
        ),
    )
