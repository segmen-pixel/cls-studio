...
# A project id is a directory name that every file in that project sits under,
# so its length is paid once per path. Windows still enforces MAX_PATH (260)
# for most APIs, and a taught image already carries the user's own filename;
# spending 36 of that budget on a canonical UUID bought nothing, since the id
# is never read by a human and never leaves this installation.
#
# 12 hex characters is 48 bits. At the scale one machine holds -- projects in
# the hundreds -- a collision is far less likely than the disk failing, and
# new_project_id() re-rolls against what is already on disk anyway.
PROJECT_ID_LEN = 12
_PROJECT_ID_RE = re.compile(
    rf"^[0-9a-f]{{{PROJECT_ID_LEN}}}$"          # current
    r"|^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}"    # UUIDs written by
    r"-[0-9a-f]{4}-[0-9a-f]{12}$",              # earlier versions
    re.IGNORECASE,
)


def looks_like_project_id(name: str) -> bool:
    """Whether a directory name is one this app would have created.

    Accepts the full UUIDs earlier versions wrote: those directories hold
    irreplaceable user data and must keep being recognised.
    """
    return bool(_PROJECT_ID_RE.match(name))


def new_project_id() -> str:
    """A short project id that is free on disk."""
    for _ in range(1000):
        candidate = uuid.uuid4().hex[:PROJECT_ID_LEN]
        if not (PROJECTS_DIR / candidate).exists():
            return candidate
    raise RuntimeError("could not find a free project id")  # pragma: no cover
...
def resolve_run_path(project_id: str, run_id: str) -> Path | None:
    ...
    def _has_model(d: Path) -> bool:
        return (d / "model.pt").exists() or (d / "anomaly_model.pkl").exists()
...
def get_path_lock(path: Path) -> threading.Lock:
    """Per-path lock for read-modify-write sequences on a file or a directory.

    The same registry ``write_json``/``write_bytes_atomic`` use, public because
    a caller sometimes has to hold it across more than one write -- e.g. the
    lazy creation of a bank's default label set, which is a read ("does one
    exist?") followed by two writes.
    """
...
def _replace_with_retry(src: Path, dst: Path) -> None:
    ...
    Delegates so there is ONE backoff schedule. This was a second copy of the
    original fixed 0.1/0.2/0.3/0.4, which is coarser than the fault it retries
    and identical for every contender -- measured at 146 hard failures per 4,000
    contended replaces, against none for the jittered schedule in ``fsio``.

    Losing the ``FileNotFoundError`` retry along the way is the point, not a
    casualty: it was already unreachable here (both callers hold the path lock
    across the temp write *and* the replace, and with the parent gone the write
    above raises first), and waiting on a source that cannot come back is what
    made the collision this module's sibling fixed take a second to report.
    """
...
# ---------------------------------------------------------------------------
# Best-run archive: preserve donor model + profile on project/run deletion
# ---------------------------------------------------------------------------
...
def library_dir() -> Path:
    """Global archive directory for best-run donor models."""
