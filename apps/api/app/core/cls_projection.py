...
from .cls_eval_cache import _eval_cache_for
...
def compute_projection(
    ...
    def _forced_for(tier: str, label: str, index: list[dict]) -> list[int]:
        ...
        for e in index:
            ...
            if start < 0 or count <= 0 or not name:
                continue
            ent = eval_cache.get(f"{tier}/{label}/{name}")
            if not ent:
                continue
            out.extend(
                start + int(li)
    ...
    if granularity == "image":
        ...
        def _image_vecs(
            ...
            for e in index:
                ...
                ent = eval_cache.get(f"{tier}/{label}/{name}") or {}
    ...
    normal_s, _, normal_name_s = _sample(normal, [], normal_names, k, normal_forced)
