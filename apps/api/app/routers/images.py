...
class BankImage(BaseModel):
    ...
    entry_id: str = Field(
        "",
        description=(
            "Store entry this row range came from. Empty on banks assembled "
            "before it was recorded. It is the only identity a row has: the "
            "store deliberately allows two entries to share a filename."
        ),
    )
    patches: int = 0
    url: str = ""
