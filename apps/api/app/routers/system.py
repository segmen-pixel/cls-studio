...
GET  /system/import  — stored format for imported source images
PUT  /system/import  — persist it (applies to the next import, not retroactively)
...
from ..core.import_format import (
    FORMATS,
    MAX_QUALITY,
    MIN_QUALITY,
...
class ImportSettingsUpdate(BaseModel):
    format: str = Field("jpg", pattern="^(raw|png|jpg)$")
    jpg_quality: int = Field(92, ge=MIN_QUALITY, le=MAX_QUALITY)


@router.get("/system/import")
def get_import_settings() -> dict[str, Any]:
    """How an imported source image is stored (raw / png / jpg)."""
    return {**read_import_settings(), "formats": list(FORMATS)}


@router.put("/system/import")
def update_import_settings(payload: ImportSettingsUpdate) -> dict[str, Any]:
    """Persist it. Applies to the NEXT import; images already on disk are
    left alone, because rewriting them would silently re-encode work an
    operator has already labelled."""
    out = save_import_settings(payload.format, payload.jpg_quality)
    return {**out, "formats": list(FORMATS)}
