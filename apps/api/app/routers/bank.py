...
def cached_evaluations() -> list[StoredImageEval]:
    ...
    Joined against the bank's own row index, which is what makes the docstring
    above true. The other three consumers of this cache -- /bank/evaluation/
    cmin, the exemplar rows and the projection -- all iterate the index and
    look the cache up; this one returned the raw cache, so a deleted critical
    image kept feeding the separation histogram, the AUROC and the Youden
    auto-threshold. Nothing rolled the fingerprint to stop it: _eval_fingerprint
    is normal-tier-only by design.
    """
    state = get_state()
    state.require_active()
    return [StoredImageEval(**v) for v in eval_cache_live_entries(state).values()]
...
def evaluate_bank_image(
    ...
    # ``id`` names the store entry the row range came from. Without it this
    # took the FIRST index entry with a matching filename, and the store
    # deliberately allows two entries to share one -- so both rows of a
    # duplicate pair were scored on the same rows and the Teach list showed
    # one image's number twice. Banks assembled before the stamp carry no id
    # and keep the name lookup.
    entry = None
    if entry_id:
        entry = next(
            (e for e in index
             if str(e.get(bank_images.INDEX_ENTRY_ID_KEY, "")) == entry_id),
            None,
        )
    if entry is None:
    ...
    # Keyed on the index ENTRY, not the filename: see eval_cache_key.
    cache_key = eval_cache_key(tier, label, entry)
    ...
    # The patches==count guard is belt and braces for banks assembled before
    # the entry-id stamp existed. It does NOT cover delete-then-reteach-
    # same-name, whatever it used to claim: on a line where every frame comes
    # off one camera at one resolution the counts always match. The key does.
    ...
    result = StoredImageEval(
        ...
        tier=tier,
        label=label,
        # Stamped so a client can key its result map on the image rather than
        # on the filename, and so the restore-from-cache path agrees with the
        # sweep it is restoring.
        entry_id=str(entry.get(bank_images.INDEX_ENTRY_ID_KEY, "")),
        patches=int(count),
...
def put_bank_capacity(
    body: BankCapacitySet,
    binding: str | None = Header(None, alias="X-Bank-Binding"),
) -> BankCapacityInfo:
    ...
    state = get_state()
    state.require_active()
    # The ceiling feeds /bank/assemble, so an unbound write here coresets
    # ANOTHER project's normal tier down on its next assemble. The client
    # has always sent the header; the route simply never read it.
    check_binding(state, binding)
    assert state.bank_dir is not None
    path = state.bank_dir / RUNTIME_CONFIG_FILE
    if path.exists():
...
@router.get("/bank/export/coreml")
def export_encoder_for_coreml(precision: str = "float32") -> FileResponse:
    """Download a Core ML encoder matching the active bank.

    Separate from ``/bank/export/edge`` because the encoder belongs to an
    (model, window, layers) triple rather than to a bank: every bank on a
    given install shares one, and embedding a few hundred megabytes in each
    package would duplicate it for no new information.

    Conversion needs macOS. That is not a stray guard -- coremltools has no
    Windows build, and a converted encoder cannot be run anywhere else, so it
    could not be checked against the bank it has to match. The export is gated
    on that check, so a machine that cannot run it declines instead.
    """
    from clscore.coreml_export import CoreMLUnavailable, export_encoder

    from ..core.cls_model_export import ENCODER_SUFFIX, encoder_args_from_meta, package_encoder

    # Request validation first: "int4" is wrong whether or not a bank is
    # active, and answering "no active project" to it sends the caller after
    # the wrong problem.
    if precision not in ("float32", "float16"):
        raise HTTPException(status_code=400, detail="precision must be float32 or float16")
    ...
    with tempfile.TemporaryDirectory() as work:
        pkg = Path(work) / "encoder.mlpackage"
        try:
            with state.lock, state.io_lock:
                args = encoder_args_from_meta(state.bank.meta)
                artifact = export_encoder(
                    dest=pkg, bank=state.bank.normal, precision=precision, **args
                )
        except CoreMLUnavailable as exc:
        ...
        tmp.close()
        try:
            package_encoder(pkg, artifact, Path(tmp.name))
        except BaseException:
            os.unlink(tmp.name)
            raise

    fname = f"{args['model_name']}-{args['window']}{ENCODER_SUFFIX}"
    return FileResponse(
        tmp.name,
        media_type="application/zip",
        filename=fname,
...
@router.get("/bank/export/openvino")
def export_encoder_for_openvino(precision: str = "fp32") -> FileResponse:
    """Download an OpenVINO IR encoder matching the active bank.

    The Intel-side counterpart of /bank/export/coreml, and the one that can be
    verified here: OpenVINO converts and runs on this platform, so the IR is
    compared against the bank before it leaves.

    fp16 is a real option here rather than a compromise -- it stores half
    precision weights and still computes in fp32 on CPU, so it halves the
    artifact for a hundredth of the error Core ML's fp16 costs.
    """
    from clscore.openvino_export import OpenVINOUnavailable, export_ir

    from ..core.cls_model_export import IR_SUFFIX, encoder_args_from_meta, package_encoder
    ...
    with tempfile.TemporaryDirectory() as work:
        ...
        try:
            with state.lock, state.io_lock:
                args = encoder_args_from_meta(state.bank.meta)
                artifact = export_ir(
                    dest_dir=out, bank=state.bank.normal, precision=precision, **args
                )
        except OpenVINOUnavailable as exc:
            raise HTTPException(status_code=501, detail=str(exc)) from exc
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        ...
        tmp.close()
        try:
            package_encoder(out, artifact, Path(tmp.name), root=Path(artifact["ir"]).stem)
        except BaseException:
            os.unlink(tmp.name)
            raise
    ...
    return FileResponse(
        tmp.name,
        media_type="application/zip",
        filename=f"{Path(artifact['ir']).stem}{IR_SUFFIX}",
        background=BackgroundTask(os.unlink, tmp.name),
    )
...
def evaluation_cmin(auto_top: int = 10) -> list[ImageCmin]:
    ...
    for key, ent in cache.items():
        ...
        if tier == "normal":
            ...
            index = bank.meta.negative_image_index.get(label, [])
        # Matched on the cache key rather than the name, so a bank holding two
        # entries with one filename resolves each to its own row range.
        entry = next(
            (e for e in index if eval_cache_key(tier, label, e) == key), None
        )
...
async def append_image(
    ...
    resolved, n_patches = await run_in_threadpool(state.leased(_teach))
    if state.active_project_id:
        touch_project(state.active_project_id)
    return AppendResult(
        tier=tier,
