...
from ..core.security import check_archive_bounds, check_archive_paths
...
# The ANNOTATE dataset's extensions — a different asset from a bank's source
# images, counted by _quick_file_count below.
...
def _first_bank_image(project_id: str):
    ...
    Projects whose images went straight into a bank have an empty annotate
    dataset, so the card's thumbnail comes from the bank instead. Which of the
    three places that image sits in — and which extensions count — is
    ``bank_images``' answer, shared with the viewer and with the count below.
    """
    return bank_images.first_bank_image(project_dir(project_id) / bank_images.BANKS_SUBDIR)
...
def _bank_counts(project_id: str) -> tuple[int, int]:
    ...
    legacy summary counted — without this the card claims "0 images" forever
    no matter how much the operator taught (2026-07-18 report).

    This used to read ``bank_meta.json`` only, while ``_first_bank_image``
    twenty lines above had already been taught to look in the feature store.
    So a project imported into the store but not yet assembled — the shape
    every current project passes through — rendered a real thumbnail directly
    above the words "0 images", and after assembling it counted only the
    ASSIGNED images and left the remainder invisible. Both halves of the card
    ask the same module now.
    """
    census = bank_images.project_census(project_dir(project_id) / bank_images.BANKS_SUBDIR)
    return census.images, census.labeled
