...
# ---- membership: the route must describe the bank it has ---------------------


def test_cached_evaluations_never_returns_an_image_the_bank_has_lost(client, project_id):
    """The live way a defect leaves a bank is /store/delete then /bank/assemble.

    Neither purge hook covers that path -- they hang off /bank/clear/{tier}
    and /bank/images/delete, which no component calls -- and the fingerprint
    cannot stand in for one: _eval_fingerprint is normal-tier-only, so
    dropping a critical image leaves it byte-identical and the orphan
    survives the delete, the assemble and every restart. It then feeds the
    ...
    rows = client.get("/api/v1/bank/evaluation/cached").json()
    assert {r["name"] for r in rows} == {"a.png", "b.png"}

    # b.png leaves the bank. Done in-place rather than through the assemble
    # route so the test needs no feature store; the resulting bank is the
    # same shape an assemble would have written.
    state = get_state()
    state.bank.remove_images("critical", "scratch", ["b.png"])
    state.mark_dirty()
    state.save_bank()

    rows = client.get("/api/v1/bank/evaluation/cached").json()
    assert {r["name"] for r in rows} == {"a.png"}, (
        "a deleted defect must stop driving the AUROC and the auto-threshold"
    )
