...
def test_host_sidecars_are_written_for_a_coreml_inspection_app(tmp_path):
    ...
    # The layout is the one the server's own extractor returns -- one feature
    # vector per patch, patch-grid first. It used to say NCHW, which described
    # a model that did not exist yet; stating it wrongly here is worse than
    # not stating it, because the encoder export is now built to match.
    assert mm["logits_layout"] == "NHWC"
    assert mm["logits_shape"] == [1, 37, 37, 16]
...
def _write_index(bank_dir, rows: int, built_rows: int, clusters: int = 4) -> None:
    """A minimal ivf_index.npz, shaped the way IvfIndex.save writes one."""
    meta = json.dumps({"seed": 0, "n_clusters": clusters, "built_rows": built_rows,
                       "int8": False, "index_basis": None})
    np.savez_compressed(
        bank_dir / IVF_INDEX_FILE,
        centroids=np.zeros((clusters, 16), dtype=np.float16),
        row_cluster=np.zeros(rows, dtype=np.int32),
        meta_json=np.frombuffer(meta.encode("utf-8"), dtype=np.uint8),
    )


def test_the_package_states_the_preprocessing_it_was_built_with(tmp_path):
    """A device that normalises differently scores every query wrongly and
    reports nothing, so the chain travels with the bank rather than in prose."""
    m = build_edge_package(
        bank=_bank(), bank_dir=tmp_path, bank_id=ROOT,
        dest=tmp_path / "b.clsedge.zip", runtime_config_file=RUNTIME_CONFIG_FILE,
    )
    with zipfile.ZipFile(tmp_path / "b.clsedge.zip") as zf:
        side = _json(zf, PREPROCESS_JSON)
    assert side == m["preprocess"]
    assert side["color_space"] == "RGB"
    assert side["layout"] == "CHW"
    assert side["pixel_divisor"] == 255.0
    # Three distinct standard deviations: any host or converter that collapses
    # them to one scalar displaces every query relative to every taught row.
    assert len(set(side["normalize"]["std"])) == 3


def test_the_package_states_how_the_image_was_tiled(tmp_path):
    m = build_edge_package(
        bank=_bank(), bank_dir=tmp_path, bank_id=ROOT,
        dest=tmp_path / "b.clsedge.zip", runtime_config_file=RUNTIME_CONFIG_FILE,
    )
    tiling = m["tiling"]
    assert tiling["window"] == m["encoder"]["window"]
    assert tiling["stride"] == m["encoder"]["stride"]
    assert tiling["grid"] == m["encoder"]["window"] // m["encoder"]["patch"]
    assert tiling["rows_per_window"] == tiling["grid"] ** 2
    # Padding and edge anchoring change which pixels a row came from, so a
    # reader that guesses "stride all the way" indexes the wrong rows.
    assert "reflect" in tiling["pad"]
    assert "anchored" in tiling["last_window"]


def test_capacity_is_unbounded_when_no_index_ships(tmp_path):
    """Without routing there is nothing for an append to invalidate."""
    m = build_edge_package(
        bank=_bank(), bank_dir=tmp_path, bank_id=ROOT,
        dest=tmp_path / "b.clsedge.zip", runtime_config_file=RUNTIME_CONFIG_FILE,
    )
    assert m["capacity"]["appendable"] is True
    assert m["capacity"]["limit"] is None


def test_capacity_states_the_ceiling_the_clustering_stops_covering(tmp_path):
    """Past built_rows * REBUILD_GROWTH the routing answers with the wrong
    candidates rather than failing, so the device is told where that is."""
    from clscore.compress import REBUILD_GROWTH

    rows = 64
    _write_index(tmp_path, rows=rows, built_rows=rows)
    m = build_edge_package(
        bank=_bank(rows=rows), bank_dir=tmp_path, bank_id=ROOT,
        dest=tmp_path / "b.clsedge.zip", runtime_config_file=RUNTIME_CONFIG_FILE,
        ivf_index_file=IVF_INDEX_FILE, nprobe=2,
    )
    cap = m["capacity"]
    assert cap["built_rows"] == rows
    assert cap["rebuild_growth"] == REBUILD_GROWTH
    assert cap["max_rows"] == int(rows * REBUILD_GROWTH)
    assert cap["rows_remaining"] == cap["max_rows"] - rows
    assert cap["windows_remaining"] == cap["rows_remaining"] // m["tiling"]["rows_per_window"]


def test_capacity_reports_no_headroom_once_the_bank_outgrew_the_index(tmp_path):
    """A bank taught past the ceiling must not advertise room it does not have."""
    _write_index(tmp_path, rows=64, built_rows=8)
    m = build_edge_package(
        bank=_bank(rows=64), bank_dir=tmp_path, bank_id=ROOT,
        dest=tmp_path / "b.clsedge.zip", runtime_config_file=RUNTIME_CONFIG_FILE,
        ivf_index_file=IVF_INDEX_FILE, nprobe=2,
    )
    assert m["capacity"]["rows_remaining"] == 0
    assert m["capacity"]["windows_remaining"] == 0
