...
- **The encoder behind a bank can now be exported for on-device inference.**
  Two formats, both from the teach tab: Core ML for Apple hardware and
  OpenVINO IR for Intel CPUs. What comes out is the encoder that built the
  bank in front of you, packaged with the preprocessing contract it was
  measured under -- a package built for one bank does not describe another,
  and the two sides have to agree down to the constants or the distances stop
  meaning the same thing.

  **Both are checked before you are given them.** The converted encoder is
  scored against the same bank the original was, and a conversion whose scores
  drift is reported as a failure instead of handed over. The failure mode here
  is not a crash: it is a number that is quietly slightly wrong, which is the
  harder one to notice and the more expensive one to trust.

  **Core ML export runs on macOS only** -- the conversion and the check both
  need it -- and the button says so anywhere else. It writes full precision by
  default. Half precision converts cleanly and looks right by every summary
  statistic, but it moves scores by about 0.1% against 0.00006% at full
  precision, and in a nearest-neighbour distance space that is the same order
  as a real defect.

  **OpenVINO IR converts and verifies on Windows**, which is where this
  usually runs. It writes fp32 by default, with fp16 available: that one
  stores half-precision weights but still computes in fp32 on the CPU, so it
  halves the file without the drift half precision costs elsewhere. OpenVINO
  itself is an installer option (`--with-openvino`), and the button is greyed
  with the reason when it is not installed.

- **You can now choose how imported images are stored: as uploaded, as PNG, or
  as JPEG.** Settings → Stored image format. Until now the format followed
  whatever the camera produced — anything a browser could display was kept as
  it arrived, and anything else (a TIFF, most often) was expanded into a
  barely-compressed PNG whether you wanted that or not. Inspecting fine surface
  defects and running a line at volume are different jobs, and they want
  different answers. JPEG is the default, and an upload that is already JPEG is
  stored unchanged rather than compressed a second time. The setting applies to
  the next import; images already stored are left as they are.

### Changed

- **The bank tab's marking tools are now on the picture, down its right
  edge.** The mark toggle sat in a row above the image and *Clear marks* in a
  row below it, so the two controls that act on the same image were at
  opposite ends of it and the image gave up both rows of height to hold them.
  Picking which defect kind you are looking at moved onto the same strip,
  since that is a decision made by looking at the picture; naming a *new* kind
  stays with the labelling controls.

- **The bank tab now shows how far into the pass you are.** Bank, label and
  assemble are drawn as one line across the top of the tab, each with the
  figure it is judged on -- images held, how many of them are labelled, and
  whether the bank still matches those labels. The numbered panels down the
  right column already ticked themselves as they finished, but that column
  scrolls, so you could be looking at none of them.

### Removed

- **Assigning a group by hand has been removed.** The box named a lot on the
  bank tab, but the switch that made the name mean anything was on the teach
  tab, so using the box on its own did nothing and nothing said so. Grouping
  itself is unchanged: *Date in the filename* and *Filename prefix* still
  derive a group and still hold a whole lot out of the separation check, which
  is what grouping is for. Groups already stored are left alone and are still
  returned by the API.

### Fixed

- **The Windows installer could fail partway through, differently each time.**
  On a machine where the newest Python is the one from the Microsoft Store, the
  installer picked it -- and a virtual environment built on that Python cannot be
  started by anything except the desktop session that created it, so the app would
  not run as a service or over a remote connection. The installer now passes over
  Store builds and uses a normally-installed Python instead; if the Store one is
  all there is, it installs a real Python as it already did for a machine with
  none.
- The installer's graphics-card check printed an error message where the card's
  name should have been, while still reporting OK. It now names the card.
- **Windows scripts are now stored with Windows line endings.** They were being
  converted only when the project was cloned, which meant the ZIP downloaded from
  a release contained the wrong ones -- and Windows' own script interpreter, given
  a file with the wrong line endings, can jump to the wrong place in it. The
  effect was an install that stopped at a different step depending on the machine,
  with a message that pointed nowhere near the cause. Nothing changes for anyone
  who already has it working.

### Changed

- **The Windows GPU install now needs an NVIDIA driver of version 580 or newer.**
  It builds on CUDA 13.0 instead of 12.8. If your driver is older than that, update
  it before upgrading, or the GPU will not be found. The range of supported graphics
  cards is unchanged -- everything from RTX 20-series through the latest generation,
  the same as before. Machines with older cards were already on a separate build and
  are not affected.

  The reason is not novelty: PyTorch stopped publishing CUDA 12.8 builds after
  version 2.11, which left the Windows GPU install stuck on a version with a
  published vulnerability and no way forward on that line.

### Security

- The Windows GPU install was pinned to versions of six libraries with published
  vulnerabilities, in exactly the places this program spends its time: reading image
  files, receiving uploads, and serving files over HTTP. That install was never
  scanned. The project checks its other dependency lists for known vulnerabilities on
  every change, but the GPU one was left out, and by the time anyone looked it was
  holding every advisory the project had. It is now scanned like the rest, and so is
  the OpenVINO one, and all of them are clear.

### Fixed

- The model could not be loaded without an internet connection, even on a
  machine that had already downloaded it. Working out where to fetch DINOv2
  from involved asking GitHub which branch to use, every single time, before
  anything looked at the local copy. A machine that had the model cached still
  failed offline, and loading several models in a row could be refused by
  GitHub for asking too often -- which surfaced as an unrelated-looking error
  from deep inside the network library rather than as "rate limited". The
  branch is now stated outright, so a machine that already has the model needs
  no network at all. Nothing re-downloads.
