...
- **Validation** decides how much is held out. The default
  *Leave-one-image-out* excludes only the image being scored, which reads
  ...
  the bank. Choose a grouping rule — by the date in the filename, by a
  filename prefix, or by a group you name yourself on the Bank tab — and
  the whole lot is held out instead.
