...
- **Assign a group** names a lot by hand for the selected images, so a
  grouped run on the Teach tab can hold that whole lot out.
...
- **Validation** decides how much is held out per image:
  ...
  - *Manual* uses the group names assigned on the Bank tab.
